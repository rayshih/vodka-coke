...
var slice    = [].slice;
var handlers = UTILS.build_hash( VODKA_HANDLER_DIR );
...
var Dispatcher = Class.extend({
  ...
  build_action : function ( next ){
    ...
    if( typeof this.handler_name === 'function'){
      var handler = this.handler_name;

      this.handler = function ( err, res, body ){
        var _next = function (){
          logger.log( UTILS.$good( ' Passed :)' ));
          next && next.apply( this, slice.call( arguments ));
        };

        handler( err, res, body, self.log, _next );
      };
  ...
  exec : function ( method ){
    ...
    request( opt, function ( err, res, body ){
      ...
      if( self.handler_class ){
        tmp = [ '[ ', self.handler_class, ' ]', '[ ', self.handler_method, ' ]' ].join( '' );
      }else{
        tmp = '[ In File ]';
      }
