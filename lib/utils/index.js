 ...
 */
  ...
  deep_inverse_merge: require( './deep_inverse_merge' )
};

/**
 * Exports module as getter to global.
 */
var origin_defineGetter = global.__defineGetter__;
var defineUTILS = function (){
  origin_defineGetter( 'UTILS', function (){
    return utils;
  });
};

defineUTILS();
global.__defineGetter__ = function ( name, method ){
  if( name !== 'UTILS' ){
    origin_defineGetter( name, method );
    return;
  }

  var ORIGIN_UTILS = method();
  if( ORIGIN_UTILS ){
    utils.deep_inverse_merge( utils, ORIGIN_UTILS );
  }

  defineUTILS();
  global.__defineGetter__ = origin_defineGetter;
};
