...
_ = {
  ...
  create : function ( path, content, method ){
    ...
    out.path_exists( path, function ( full_path ){
      logger.log( utils.$fine( 'exists' ) + '  ' + path );
    }, function ( full_path ){
      fs[ method ]( full_path, content );
      logger.log( utils.$good( 'create' ) + '  ' + path );
    });
  },
 ...
 */
  ...
  is_args_defined : function ( args, msg ){
    if( args === undefined || args.length === 0 ){
      ...
      logger.log( 'Usage example: vodka ' + msg );
   ...
   * search_project_root
   *
   * @param callback {Function} callback( root_path )
   * @return
   */
  search_project_root : function ( callback ){
    var current_dir = process.cwd();

    if( current_dir === '/' ) return callback( null );

    if( fs.existsSync( current_dir + '/server.js' )){
      return callback( current_dir );
    }

    // If not found
    process.chdir( '..' );
    this.search_project_root( callback );
  },
 ...
 */
  ...
  create_file_f : function ( path, content ){
    ...
    logger.log( utils.$update( 'update' ) + '  ' + path );
  },
